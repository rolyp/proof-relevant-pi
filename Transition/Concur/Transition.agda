...
module Transition.Concur.Transition where
   ...
   /-preserves-⌣ᵁᴰᵁ 𝐸 𝐸′ 𝐸″ with /-preserves-⌣ (⌣-sym 𝐸) 𝐸′ (⌣-sym 𝐸″)
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᴰᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah

   /-preserves-⌣ᵁᵁᵁ 𝐸 𝐸′ 𝐸″ with /-preserves-⌣ (⌣-sym 𝐸) (⌣-sym 𝐸′) (⌣-sym 𝐸″)
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_] {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_] {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} _) | blah = blah
   /-preserves-⌣ᵁᵁᵁ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) _ ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} _) | blah = blah
