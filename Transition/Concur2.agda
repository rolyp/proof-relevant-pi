module Transition.Concur2 where
   ...
   nibble : ∀ {Γ P} {a a′ : Action Γ} {a⌣a′ : a ᴬ⌣ a′} {R R′} {E : P —[ a - _ ]→ R} {E′ : P —[ a′ - _ ]→ R′} →
            (𝐸 : E ⌣[ a⌣a′ ] E′) → E/E′ (⊖ 𝐸) ≅ E′/E (⊖ (⌣-sym 𝐸))
   nibble ([_] {a⌣a′ = ᵛ∇ᵛ} 𝐸) = ≅-refl
   nibble ([_] {a⌣a′ = ᵇ∇ᵇ} 𝐸) = ≅-refl
   nibble ([_] {a⌣a′ = ᵇ∇ᶜ} 𝐸) = ≅-refl
   nibble ([_] {a⌣a′ = ᶜ∇ᵇ} 𝐸) = ≅-refl
   nibble ([_] {a⌣a′ = ᶜ∇ᶜ} 𝐸) = ≅-refl
   nibble ([_]ˡ {a⌣a′ = ᵛ∇ᵛ} 𝐸) = ≅-refl
   nibble ([_]ˡ {a⌣a′ = ᵇ∇ᵇ} 𝐸) = ≅-refl
   nibble ([_]ˡ {a⌣a′ = ᵇ∇ᶜ} 𝐸) = ≅-refl
   nibble ([_]ˡ {a⌣a′ = ᶜ∇ᵇ} 𝐸) = ≅-refl
   nibble ([_]ˡ {a⌣a′ = ᶜ∇ᶜ} 𝐸) = ≅-refl
