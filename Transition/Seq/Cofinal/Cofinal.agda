module Transition.Seq.Cofinal.Cofinal where
   ...
   open import Braiding.Proc as ᴮ using (_⋉_; ⋉̂-to-⋉; tgt)
   ...
   ... | _Δ_ {._} refl E⋆/γ/E | γ⋆′ =
      ...
      begin
         Proc↱ (+-assoc (Γ + 1) Δ′ (1 + Γ′)) (Proc↱ (+-assoc (Γ + 1 + Δ′) 1 Γ′) (tgt⋆ E⋆))
      ≅⟨ Proc↲ (+-assoc (Γ + 1) Δ′ (1 + Γ′)) _ ⟩
         Proc↱ (+-assoc (Γ + 1 + Δ′) 1 Γ′) (tgt⋆ E⋆)
      ≅⟨ Proc↲ (+-assoc (Γ + 1 + Δ′) 1 Γ′) _ ⟩
         tgt⋆ E⋆
      ...
      ≅⟨ γ⋆′ ⟩
         tgt⋆ E⋆/γ/E
      ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + 1 + Δ′) 1 Γ′) _) ⟩
         Proc↱ (+-assoc (Γ + 1 + Δ′) 1 Γ′) (tgt⋆ E⋆/γ/E)
      ∎
   ...
   ... | _Δ_ {._} refl E⋆/γ/E | γ⋆′ =
      let open ≅-Reasoning; Γ′ = inc⋆ a⋆ in
      begin
         Proc↱ (+-assoc (Γ + 1) Δ′ (0 + Γ′)) (Proc↱ (+-assoc (Γ + 1 + Δ′) 0 Γ′) (tgt⋆ E⋆))
      ≅⟨ Proc↲ (+-assoc (Γ + 1) Δ′ (0 + Γ′)) _ ⟩
         Proc↱ (+-assoc (Γ + 1 + Δ′) 0 Γ′) (tgt⋆ E⋆)
      ≅⟨ Proc↲ (+-assoc (Γ + 1 + Δ′) 0 Γ′) _ ⟩
         tgt⋆ E⋆
      ...
      ≅⟨ γ⋆′ ⟩
         tgt⋆ E⋆/γ/E
      ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + 1 + Δ′) 0 Γ′) _) ⟩
         Proc↱ (+-assoc (Γ + 1 + Δ′) 0 Γ′) (tgt⋆ E⋆/γ/E)
      ∎
   γ⋆ ᵇ∇ᵇ _ [] _ = ≅-refl
   ...
   ... | _Δ_ {._} refl E⋆/γ/E | γ⋆′ =
      ...
      begin
         ((swap ᴿ+ (Δ′ + (1 + Γ′))) *)
         (Proc↱ (+-assoc (Γ + 2) Δ′ (1 + Γ′)) (Proc↱ (+-assoc (Γ + 2 + Δ′) 1 Γ′) (tgt⋆ E⋆)))
      ≅⟨ ≅-cong-swap (sym (+-assoc Δ′ 1 Γ′))
         (begin
            Proc↱ (+-assoc (Γ + 2) Δ′ (1 + Γ′)) (Proc↱ (+-assoc (Γ + 2 + Δ′) 1 Γ′) (tgt⋆ E⋆))
         ≅⟨ Proc↲ (+-assoc (Γ + 2) Δ′ (1 + Γ′)) _ ⟩
            Proc↱ (+-assoc (Γ + 2 + Δ′) 1 Γ′) (tgt⋆ E⋆)
         ≅⟨ Proc↲ (+-assoc (Γ + 2 + Δ′) 1 Γ′) _ ⟩
            tgt⋆ E⋆
         ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + 2) (Δ′ + 1) Γ′) _) ⟩
            Proc↱ (+-assoc (Γ + 2) (Δ′ + 1) Γ′) (tgt⋆ E⋆)
         ∎) ⟩
         ((swap ᴿ+ (Δ′ + 1 + Γ′)) *) (Proc↱ (+-assoc (Γ + 2) (Δ′ + 1) Γ′) (tgt⋆ E⋆))
      ≅⟨ γ⋆′ ⟩
         tgt⋆ E⋆/γ/E
      ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + 2 + Δ′) 1 (inc⋆ ((suc (swap ᴿ+ Δ′) *) a⋆))) _) ⟩
         Proc↱ (+-assoc (Γ + 2 + Δ′) 1 (inc⋆ ((suc (swap ᴿ+ Δ′) *) a⋆))) (tgt⋆ E⋆/γ/E)
      ∎
   ...
   ... | _Δ_ {._} refl E⋆/γ/E | γ⋆′ =
      ...
      begin
         ((swap ᴿ+ (Δ′ + (0 + Γ′))) *)
         (Proc↱ (+-assoc (Γ + 2) Δ′ (0 + Γ′)) (Proc↱ (+-assoc (Γ + 2 + Δ′) 0 Γ′) (tgt⋆ E⋆)))
      ≅⟨ ≅-cong-swap (sym (+-assoc Δ′ 0 Γ′))
         (begin
            Proc↱ (+-assoc (Γ + 2) Δ′ (0 + Γ′)) (Proc↱ (+-assoc (Γ + 2 + Δ′) 0 Γ′) (tgt⋆ E⋆))
         ≅⟨ Proc↲ (+-assoc (Γ + 2) Δ′ (0 + Γ′)) _ ⟩
            Proc↱ (+-assoc (Γ + 2 + Δ′) 0 Γ′) (tgt⋆ E⋆)
         ≅⟨ Proc↲ (+-assoc (Γ + 2 + Δ′) 0 Γ′) _ ⟩
            tgt⋆ E⋆
         ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + 2) (Δ′ + 0) Γ′) _) ⟩
            Proc↱ (+-assoc (Γ + 2) (Δ′ + 0) Γ′) (tgt⋆ E⋆)
         ∎) ⟩
         ((swap ᴿ+ (Δ′ + 0 + Γ′)) *) (Proc↱ (+-assoc (Γ + 2) (Δ′ + 0) Γ′) (tgt⋆ E⋆))
      ≅⟨ γ⋆′ ⟩
         tgt⋆ E⋆/γ/E
      ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + 2 + Δ′) 0 (inc⋆ (((swap ᴿ+ Δ′) *) a⋆))) _) ⟩
         Proc↱ (+-assoc (Γ + 2 + Δ′) 0 (inc⋆ (((swap ᴿ+ Δ′) *) a⋆))) (tgt⋆ E⋆/γ/E)
      ∎
   γ⋆ ᵇ∇ᶜ _ [] _ = ≅-refl
   ...
   ... | φ/E/E⋆ Δ E⋆/φ/E | γ⋆′ =
      let open ≅-Reasoning; Γ′ = inc⋆ a⋆ in
      begin
         Proc↱ (cong (_+_ Γ) (+-assoc Δ′ 1 Γ′)) (tgt φ/E/E⋆)
      ≅⟨ Proc↲ (cong (_+_ Γ) (+-assoc Δ′ 1 Γ′)) _ ⟩
         tgt φ/E/E⋆
      ≅⟨ γ⋆′ ⟩
         tgt⋆ E⋆/φ/E
      ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + Δ′) 1 Γ′) _) ⟩
         Proc↱ (+-assoc (Γ + Δ′) 1 Γ′) (tgt⋆ E⋆/φ/E)
      ∎
   ...
   ... | φ/E/E⋆ Δ E⋆/φ/E | γ⋆′ =
      let open ≅-Reasoning; Γ′ = inc⋆ a⋆ in
      begin
         Proc↱ (cong (_+_ Γ) (+-assoc Δ′ 0 Γ′)) (tgt φ/E/E⋆)
      ≅⟨ Proc↲ (cong (_+_ Γ) (+-assoc Δ′ 0 Γ′)) _ ⟩
         tgt φ/E/E⋆
      ≅⟨ γ⋆′ ⟩
         tgt⋆ E⋆/φ/E
      ≅⟨ ≅-sym (Proc↲ (+-assoc (Γ + Δ′) 0 Γ′) _) ⟩
         Proc↱ (+-assoc (Γ + Δ′) 0 Γ′) (tgt⋆ E⋆/φ/E)
      ∎
